...
4. Routes by query param: `?shared-with=<teacher>` (students shared with that
   teacher), `?shared` (overview of all sharing teachers),
   `?mode=learn|review|browse[&scope=&id=]` (study/browse deep link), `?learn`
   (the class face grids), default (two-box menu). A model with no classes
   renders `landing.html` ("contact Mr. Seibel") instead. The model + route +
   context land in `index.html` as JSON (via `<?!= jsonForScript(...) ?>`) for
...
- First tab — student rows (headers: `studentNumber`, `firstName`,
  ...
  `teacherEmail`, `personId`, `grade`, `teacherName`, `room`). The student data
...
admin view-as form and impersonation banner — and the help overlay).
