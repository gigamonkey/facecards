 ...
 *   - `staff overrides` tab : owner-maintained corrections to staff entries
 *                  (name/role fields, a photo URL or data URL, and an opt-out
 *                  flag), edited by hand in the spreadsheet
  ...
  return cachedBig('staff', function () {
    ...
    var staff = readStaff().map(function (r) {
      ...
      return {
        ...
        email: email,
        // An overrides-tab photo (a URL or data URL) wins over the scraped
        // directory URL; both are plain <img> srcs to the client.
 ...
 * photo change can take up to the cache TTL to appear there.)
 */
...
/**
 * Corrections from the `staff overrides` tab (headers: email, firstName,
 * lastName, role, photo, optOut), keyed by normalized email. Edited by hand
 * in the spreadsheet (run dropBigCache('staff') or clearCaches() after). A
 * separate tab from the scraped `staff` one so refreshStaffDirectory can
 * rewrite that wholesale without losing the corrections. A non-empty
