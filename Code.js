 ...
 *                  (name/role fields, plus a self-taken photo as a small JPEG
 *                  data URL), written from the ?staff-edit form
 *
...
function doGet(e) {
  ...
  if (params.staff !== undefined) {
    ...
    sf.staffJson = jsonForScript(buildStaffModel());
  ...
  // client picks the viewer's row out of the staff model by ctx.effective.
  if (params['staff-edit'] !== undefined) {
    ...
    se.staffJson = jsonForScript(buildStaffModel());
  ...
  t.staffJson = jsonForScript(route.scope === 'staff' ? buildStaffModel() : null);
...
var STAFF_OVERRIDE_FIELDS = ['firstName', 'lastName', 'role', 'photo'];

/**
 * Merge `changes` (any subset of firstName/lastName/role/photo; '' clears a
 * field, an absent key leaves it alone) into email's row of the `staff
 * overrides` tab, dropping the row entirely when nothing is left overridden.
...
function updateStaffOverride(email, changes) {
  ...
  try {
    ...
    sheet.clearContents();
    sheet.getRange(1, 1, 1, 5).setValues([['email'].concat(STAFF_OVERRIDE_FIELDS)]);
