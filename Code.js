 ...
 *                  written by saveStaffOverride() (via the ?staff-edit form)
 *
  ...
  return cachedBig('staff', function () {
    ...
    var staff = readStaff().map(function (r) {
      ...
      return {
        ...
        overridden: !!o, // the edit form shows its Reset button off this
      };
    });
...
function saveStaffOverride(fields, as) {
  ...
  readStaff().forEach(function (r) {
    if (!base && normalizeEmail(r.email) === email) base = r;
  });
  ...
  rewriteStaffOverride(email, same ? null : { firstName: firstName, lastName: lastName, role: role });
  dropBigCache('staff');
...
/**
 * Rewrite the `staff overrides` tab with email's row replaced by `fields`
 * (null = remove it). A read-modify-write of the whole tab — it's small — so
 * a script lock serializes concurrent saves (same pattern as rewriteLists).
 */
function rewriteStaffOverride(email, fields) {
  ...
  try {
    ...
    var rows = sheetToObjects(sheet)
      .filter(function (r) {
        return normalizeEmail(r.email) !== target;
      })
      .map(function (r) {
        return [String(r.email), String(r.firstName || ''), String(r.lastName || ''), String(r.role || '')];
      });
    if (fields) rows.push([target, fields.firstName, fields.lastName, fields.role]);
    sheet.clearContents();
    sheet.getRange(1, 1, 1, 4).setValues([['email', 'firstName', 'lastName', 'role']]);
