...
3. `buildModel(effective)` reduces the roster to `{ teacherLast, classes }`
   **scoped to only that email's own sections** (rows where `teacherEmail`
   ...
   also carries their full cross-teacher `schedule` (for browse mode). A
   counselor's caseload (from the `counselors` tab) appears as extra
   class-like groups in `classes`, one per grade ("Grade 9", slug
   `<username>-g9`), so grids, study modes, deep links, and `hasOwn` treat
   them as classes. Shared views treat counseling as sharing, both
   directions: a counselor's `?shared` lists their advisees' teachers, and a
   teacher's `?shared` includes counselors of their students (labeled
   "Counselor", resolvable as `?shared-with=` targets).
...
- `rosters` — student rows (headers: `studentNumber`, `firstName`,
  ...
  `teacherEmail`, `personId`, `grade`, `teacherName`, `room`). The owner can
  replace the whole tab in-app: the navbar's Roster button uploads a CSV/TSV
  export (`uploadRoster` — validates headers and a row-count floor before
  touching the tab, then clears and rewarms the caches itself).
- `counselors` — one row per student/counselor pairing (headers:
  `studentNumber`, `counselorEmail`, and `counselorName` — "Last, First"
  like the roster's `teacherName`; optional but shown to teachers, so
  include it). Same in-app replacement as the roster: the navbar's
  Counselors button (`uploadCounselors`, same guards and cache rewarm).
  Hand edits need `clearCaches()`. Missing tab = no counselors, no errors.
- `photos`, `missing photos` — generated by `refreshPhotoMap()`.
- `admins` — one admin `email` per row (seed with `peterseibel@berkeley.net`).
- `aliases` — primary (IC/roster) email in the first column, alias in the
  second, for staff whose public-directory email is a Workspace alias of
  their login/roster address. Directory emails are canonicalized to the
  primary (`readAliases`/`canonicalStaffEmail`) before any join — courses,
  overrides, shared-view photos, `?as=` — so both addresses work. Cached:
  run `clearCaches()` after editing the tab.
...
showing) → `js-shared` (shared-students views: the ?shared overview — a table of
sharing teachers, big staff photo linking to their shared-with page plus
name/courses/shared-count, photos joined server-side from the public
